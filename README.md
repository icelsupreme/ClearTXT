...
- **Configurable fixes.** The "Fixes & explanation" drawer lists every transformation as an independent toggle, grouped into **Typography & normalization** (Unicode normalization, smart quotes, em dash conversion), **Languages & scripts** (accent folding, Hebrew, Cyrillic), **Symbols & invisible characters** (emoji/symbols, currency symbols, invisible/control characters), and **Whitespace cleanup** (tabs, extra spaces, line breaks, paragraph breaks) - each group can be toggled at once, and "Restore defaults" resets everything in one click. A fix that's off leaves its characters untouched.
