...
### 1.10.1
- **Docs:** added a "Deceptive fonts" section to [documentation.html](docs/documentation.html), explaining a technique researched but deliberately not built as a toggle: redefining a custom font's own glyph mapping so a character *displays* as something other than what it actually is, making text read one way to a human's eyes and another way to anything reading the underlying Unicode (including an AI tool). Unlike every other protection on that page, this isn't something ClearTXT strips, because a font is never part of copied/pasted text - only characters are - so text pasted here always renders in ClearTXT's own typeface, never whatever font the original source specified. This is disclosed as an inherent property of how the app already works (plain text in, ClearTXT's own font out), not a new fix.

### 1.10.0
- **Security feature:** a new "Strip unsafe link protocols" toggle, on by default, that removes a Markdown link's target - keeping the link's own visible display text in place - when it points somewhere other than `http:`, `https:`, `mailto:`, or `tel:`. Prompted by CVE-2026-20841 (Windows Notepad's new Markdown mode): clicking a link with an unexpected/unverified protocol let it load or run a local or remote file with no further warning, a risk that applies equally to any Markdown-rendering app a "cleaned" file might later be opened in (chat apps, note-taking apps, IDE previews, ...), not just Notepad specifically. A link with no scheme at all - a relative path (`./page.html`) or same-page fragment (`#section`) - can't invoke a protocol handler either way and is always left alone; scheme matching is case-insensitive. Image references (`![alt](url)`) are deliberately excluded - an image isn't something you click to invoke a protocol the way a link is.
  - This is the first ClearTXT toggle to touch Markdown link *syntax* rather than pure character-level content - previously the app's stated design (see `stripMarkdown`'s own doc comment) deliberately left all Markdown syntax untouched in the real output, only stripping it in an internal word-count-only copy. This toggle is a narrow, additive exception to that: it only ever removes a link's target text (never its display text, never any other Markdown construct), and only when the target's scheme fails the small allowlist above.
  - Implementation note: link/target matching is a hand-rolled, paren-depth-aware bracket scanner over the same code-point array the rest of `processText` already operates on (not a regex run against the raw string), both to stay consistent with how `findMixedScriptIndices` already avoids UTF-16-code-unit-vs-code-point index mismatches, and because a URL scheme like `javascript:` routinely contains its own parenthesized arguments (`javascript:alert(1)`) that a naive "stop at the next `)`" scan would cut short.
  - Trade-off worth calling out, consistent with this project's practice of disclosing them: this is pragmatic bracket/paren matching, not a full CommonMark parser (matching `stripMarkdown`'s own stated level of rigor), so plain prose that coincidentally looks like `[text](something:with-a-colon)` - not intended as a Markdown link at all - can be caught by the same check. The toggle can be turned off if this is a problem for your text.
